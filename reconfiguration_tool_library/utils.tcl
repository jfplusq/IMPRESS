...
namespace eval ::reconfiguration_tool::utils {
  ...
  proc place_no_placement_static_pblocks {} {
    ...
    puts "no_placement_pblock_list $no_placement_pblock_list"
  ...
  proc get_edge_INT_tiles {pblock_name {expansion 0}} {
    set tiles_pblock [get_tiles -of_objects [get_sites -of_objects [get_pblocks *$pblock_name*]]]
    ...
    foreach X_property $X_properties {
      ...
      lappend NORTH_INT_tiles [get_tiles -filter "TYPE =~ INT_? && INT_TILE_X == $X_property && INT_TILE_Y == [expr [lindex $Y_property_of_column_tiles [expr [llength $Y_property_of_column_tiles] -1]] - $expansion]"]
    }
